...
@router.get('/', responde_model=List[ShowUser], status_code=status.HTTP_200_OK)
...
def crear_usuario(usuario:User, db:Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    res = user.crear_usuario(usuario, db)
    return res

@router.get('/{user_id}', responde_model=ShowUser, status_code=status.HTTP_200_OK)
